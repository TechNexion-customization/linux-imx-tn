...
static void sec_mipi_dsim_set_main_mode(struct sec_mipi_dsim *dsim)
{
	...
	*/
	mhporch |= MHPORCH_SET_MAINHFP(/*hfp_wc*/vmode->hfront_porch*(bit_clk/8)/pixel_clk /2) |
		   MHPORCH_SET_MAINHBP(/*hbp_wc*/vmode->hback_porch * (bit_clk/8)/pixel_clk);
	...
	msync |= MSYNC_SET_MAINVSA(vmode->vsync_len) |
		...
		MSYNC_SET_MAINHSA(/*hsa_wc*/vmode->hsync_len * (bit_clk/8)/pixel_clk);
